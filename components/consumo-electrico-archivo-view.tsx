...
interface ConsumoElectricoFile {
  id: string;
  filename: string;
  file_url: string;
  created_at: string;
...
export function ConsumoElectricoArchivoView() {
  const [files, setFiles] = useState<ConsumoElectricoFile[]>([]);
  ...
  useEffect(() => {
    async function fetchFiles() {
      try {
        const response = await fetch("/api/archivo/consumo-electrico-files");
        const result = await response.json();

        if (result.error) {
          console.error("Error fetching consumo electrico files:", result.error);
        } else {
          setFiles(result.files || []);
        }
      } catch (error) {
        console.error("Error fetching consumo electrico files:", error);
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleDelete = async (fileId: string, filename: string) => {
    ...
    try {
      const response = await fetch(`/api/archivo/consumo-electrico-files/${fileId}`, {
        method: "DELETE",
      });
  ...
  if (files.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12">
        ...
        <p className="text-gray-600">No hay archivos de consumo eléctrico disponibles</p>
      </div>
    );
  }
