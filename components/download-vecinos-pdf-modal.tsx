...
interface DownloadVecinosPdfModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export function DownloadVecinosPdfModal({ isOpen, onClose }: DownloadVecinosPdfModalProps) {
  ...
  const handleDownload = async () => {
    ...
    try {
      const response = await fetch("/api/vecinos/download-pdf", {
        method: "POST",
      ...
      if (result.pdfUrl) {
        ...
        link.download = result.filename || "estado-cuenta-vecinos.pdf";
        ...
        <div className="px-6 py-5">
          <label className="flex items-center gap-2.5 cursor-pointer rounded-[14px] bg-white border border-[#E9E2CE] px-3 py-2.5 hover:border-[#2d5016]/40 transition-colors">
            ...
            <span className="text-xs text-[#1a2617]">
              Guardar copia en Archivos → Estados de Cuentas
            </span>
          </label>
